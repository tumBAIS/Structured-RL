...
dvsp_final[4] = vcat([repeat([x], 10) for x in dvsp_final[4]]...);
dvsp_final[5] = vcat([repeat([x], 10) for x in dvsp_final[5]]...);
dvsp_final[9] = vcat([repeat([x], 10) for x in dvsp_final[9]]...);
dvsp_final[10] = vcat([repeat([x], 10) for x in dvsp_final[10]]...); # Repition to account for higher number of tests for algorithms
